...
\begin{figure}[htbp]
  \centering
  % Include the figure if available; otherwise show a placeholder box to avoid compile failure
  \IfFileExists{interface_angle_pdf.pdf}{%
    \includegraphics[width=4in]{interface_angle_pdf.pdf}%
  }{%
    \fbox{\parbox[b][2in][c]{4in}{\centering Figure placeholder: interface\_angle\_pdf.pdf not found}}%
  }
...
% Use explicit relative path so bibtex finds the file when building from repo root
\bibliography{reply_to_reviewers/references}
\end{document}
